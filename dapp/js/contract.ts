...
export class AlarmContract extends SmartContract {
  ...
  public async deposit(amount: string, address: String): Promise<string> {
    ...
    if (!tpInfo.isConnected()) {
      hash = await super.moac.sendTransactionWithCallData(process.env.MOAC_SECRET, process.env.CONTRACT, amount, bytes, { gasLimit: 180000 });
    } else {
      hash = await this.sendTransactionByTp(process.env.CONTRACT, amount, bytes, { gasLimit: 180000 });
    }
    return hash;
  }
