<template>
    <div ref="scroll" flex class="scroll-wrapper" style="height: calc(100%-1.1rem);">
         <div flex-box="1" flex="dir:top cross: center">
              <div style="text-align:left;margin-top: 0.2rem;">{{ $t("moac_wallet") }}</div>
             <div style="word-break:break-all;text-align:left;margin-top:0.1rem;">{{ address }}</div>
             <van-field v-model="moacContractAddress" center type="text" :placeholder="$t('input_deposit_maoc_address')" style="margin-top: 0.3rem;"/>
            <div flex="main:justify cross:center" class="jcc-moac-alarm-balance-text" style="margin-top: 0.3rem;">{{ $t("your_wallet_balance", { amount:  isNaN(parseFloat(balance))?'--':parseFloat(balance).toFixed(8), token: $t("moac") }) }}</div>
            <div flex="main:justify cross:center" class="jcc-moac-alarm-balance-text" style="margin-top: 0.3rem;">{{ $t("contract_address_balance", { amount: isNaN(parseFloat(contractAddrBalance))?'--':parseFloat(contractAddrBalance).toFixed(8), token: $t("moac") }) }}</div>
